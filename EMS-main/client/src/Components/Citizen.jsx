...
function Signup() {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://ems-g120-api.vercel.app/register', formData)
      .then(result => {
        console.log(result);
        login();
        navigate('/');
  ...
  const handleGoogleSignIn = (credentialResponse) => {
    ...
    axios.post('https://ems-g120-api.vercel.app/google-signin', { googleData: credentialResponseDecoded })  // <-- Add the trailing slash
      .then(result => {
        console.log(result);
        navigate('/');
      })
